...
def calc_seq_err_robust(pred_bb, anno_bb, dataset, target_visible=None):
    ...
    begin_index = np.where(anno_bb.any(1) == True)[0][0]
    pred_bb = pred_bb[begin_index:,:]
    anno_bb = anno_bb[begin_index:,:]
    pred_bb[0, :] = anno_bb[0, :]
...
def extract_results(trackers, dataset, report_name,result_plot_path, skip_missing_seq=False, plot_bin_gap=0.05,
