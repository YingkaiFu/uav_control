...
import torch
from data.dataset.dataset_base import DatasetBase
...
def load_pth_between(img_file1: str,img_file2:str) -> np.array:
    path = Path(img_file1)
    path2 = Path(img_file2)
    s1 = int(path.stem)
    s2 = int(path2.stem)
    a = s1 -s2
    if a ==0:
        out2 = torch.load(img_file1)
    elif a <0:
        output = torch.stack([torch.load(path.parent / (str(f).zfill(5) + '.pth')) for f in range(s1, s2)])
        out2 = torch.sum(output, dim=0)
    else:
        output = torch.stack([torch.load(path.parent / (str(f).zfill(5) + '.pth')) for f in range(s2, s1)])
        out2 = torch.sum(output, dim=0)
        out2 = -out2
    numpy_image = (out2).numpy().transpose(1,2,0)
    # numpy_image = (numpy_image-np.min(numpy_image))/(np.max(numpy_image)-np.min(numpy_image))*255
    output = np.concatenate((numpy_image, numpy_image, numpy_image), axis=-1)
    return output
...
class TrackPairSampler(SamplerBase):
    ...
    def __getitem__(self, item) -> dict:
        ...
        while self.data_filter(data1) or self.data_filter(data2):
            ...
            event1 = data1["event"]
            # data1["image"] = load_pth(data1["image"])
            data1["event"] = load_pth(data1["event"])
            # data2["image"] = load_pth(data2["image"])
            data2["event"] = load_pth(data2["event"])
            sample_try_num += 1
        sampled_data = dict(
            data1=data1,
            data2=data2,
    ...
    def _sample_pair_idx_pair_within_max_diff(self, L, max_diff):
        ...
        rng = self._state["rng"]
        idx1 = rng.choice(L)
        idx2_choices = list(set(range(idx1-max_diff, L)).intersection(set(range(0, idx1+max_diff+1))))
