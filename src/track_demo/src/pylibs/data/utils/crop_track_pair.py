...
from pipeline.utils.bbox import cxywh2xyxy, xyxy2cxywh
from pipeline.utils.crop import get_subwindow_tracking
...
def crop_track_pair(
    ...
    if avg_chans is None:
        avg_chans2 = np.mean(em_temp,axis=(0, 1))
    ...
    if mask_tmp is not None:
        em_z, mask_ez = get_subwindow_tracking(em_temp,
                                              box_crop_temp[:2],
                                              z_size,
                                              s_temp,
        ...
        em_z = get_subwindow_tracking(em_temp,
                                      box_crop_temp[:2],
                                      z_size,
                                      s_temp,
                                      avg_chans=avg_chans2)
    if mask_curr is not None:
        em_x, mask_ex = get_subwindow_tracking(em_curr,
                                              box_crop_curr[:2],
                                              x_size,
                                              s_curr,
                                              avg_chans=avg_chans2,
        ...
        em_x = get_subwindow_tracking(em_curr,
                                      box_crop_curr[:2],
                                      x_size,
                                      s_curr,
                                      avg_chans=avg_chans2)

    return em_z, bbox_z,em_x, bbox_x, mask_z, mask_x
