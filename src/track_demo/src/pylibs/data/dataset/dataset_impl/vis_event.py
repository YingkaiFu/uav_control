...
from data.dataset.dataset_base import TRACK_DATASETS, DatasetBase
from evaluation.got_benchmark.datasets import Vis
from pipeline.utils.bbox import xywh2xyxy

_current_dir = osp.dirname(osp.realpath(__file__))


@TRACK_DATASETS.register
class VisDataset(DatasetBase):
    r"""
    ...
    def update_params(self):
        ...
        self._state["dataset"] = Vis(dataset_root,
                                        subset=subset,
                                        check_integrity=check_integrity)

    def __getitem__(self, item: int) -> Dict:
        event_files, anno = self._state["dataset"][item]

        anno = xywh2xyxy(anno)
        sequence_data = dict(anno=anno,event = event_files)
...
@TRACK_DATASETS.register
class VisDatasetFixed(VisDataset):
    ...
    def __init__(self) -> None:
        super(VisDatasetFixed, self).__init__()

    def update_params(self):
        r"""
        an interface for update params
        """
        super(VisDatasetFixed, self).update_params()
        unfixed_list_file = self._hyper_params["unfixed_list"]
        self._state["unfixed_list"] = self._read_unfixed_list(unfixed_list_file)

    def __getitem__(self, item: int) -> Dict:
        sequence_data = super(VisDatasetFixed, self).__getitem__(item)
...
VisDatasetFixed.default_hyper_params = copy.deepcopy(
    VisDatasetFixed.default_hyper_params)
VisDatasetFixed.default_hyper_params.update(
    VisDatasetFixed.extra_hyper_params)
