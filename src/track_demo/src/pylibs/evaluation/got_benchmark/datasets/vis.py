...
class Vis(object):
    ...
    def __getitem__(self, index):
        ...
        if isinstance(index, int):
            ...
            seq_name = index
        event_files = Vis.data_dict[self.subset][seq_name]["event_files"]

        anno = Vis.data_dict[self.subset][seq_name]["anno"]

        if self.subset == 'test' and (anno.size // 4 == 1):
            anno = anno.reshape(-1, 4)
            # anno = anno[np.newaxis, :]
        else:
            assert len(event_files) == len(anno)


        if self.return_meta:
            meta = Vis.data_dict[self.subset][seq_name]["meta"]
            return event_files, anno, meta
        else:
            return event_files, anno
    ...
    def _check_cache_for_current_subset(self) -> bool:
        ...
        """
        is_valid_data_dict = (self.subset in Vis.data_dict) and \
                             (set(Vis.data_dict[self.subset].keys()) == set(self.seq_names))
    ...
    def _load_cache_for_current_subset(self, cache_path: str):
        ...
        with open(cache_path, "rb") as f:
            Vis.data_dict[self.subset] = pickle.load(f)
        logger.info("{}: loaded cache file {}".format(Vis.__name__,
                                                      cache_path))
    ...
    def _build_cache_for_current_subset(self):
        ...
        for seq_name in tqdm(self.seq_names):
            seq_dir = os.path.join(root_dir, subset, seq_name)
            event_files, anno, meta = self.load_single_sequence_img_event(seq_dir)
            Vis.data_dict[self.subset][seq_name] = dict(event_files=event_files,
                                                           anno=anno,
                                                           meta=meta)
        with open(self.cache_path, "wb") as f:
            pickle.dump(Vis.data_dict[self.subset], f)
        logger.info("{}: dump cache file to {}".format(Vis.__name__,
                                                       self.cache_path))

    def load_single_sequence(self, seq_dir):
        # img_files = sorted(glob.glob(os.path.join(seq_dir,'event','*.jpg')))
        img_files = sorted(glob.glob(os.path.join(seq_dir,'event','*.jpg')))
    ...
    def load_single_sequence_img_event(self, seq_dir):
        event_files = sorted(glob.glob(os.path.join(seq_dir,'240_event','*.pth')))

        anno = np.loadtxt(os.path.join(seq_dir, "groundtruth.txt"),
                          delimiter=',')
        if self.subset=='train':
            if os.path.exists(os.path.join(seq_dir,'absent_label.txt')):
                absent_label = os.path.join(seq_dir,'absent_label.txt')
                absent = np.loadtxt(absent_label)
                event_files = [e for e,z in zip(event_files,absent) if z==1 ]
                anno = anno[absent==1]
        if self.subset == 'test' and anno.ndim == 1:
            assert len(anno) == 4
            anno = anno[np.newaxis, :]
