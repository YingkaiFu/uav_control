...
def load_pth(img_file: str) -> np.array:
    ...
    numpy_image = torch.abs(start_event).numpy().transpose(1,2,0)
...
class ExperimentGOT10k(object):
    ...
    def run(self,
        ...
        for s in range(start_idx, end_idx):
            ...
            for r in range(self.repetitions):
                ...
                # tracking loop
                boxes, times = tracker.track(events,
                                             anno,
                                             visualize=visualize)
            ...
            record_files = glob.glob(
                os.path.join(self.result_dir, tracker.name, seq_name,
                             '%s_[0-9]*.txt' % seq_name))
            ...
            seq_ious = [
                rect_iou(b, anno) for b in boxes
            ]
            seq_ious = np.concatenate(seq_ious)

            # stack all tracking times
            times[seq_name] = []
            time_file = os.path.join(self.result_dir, tracker.name, seq_name,
                                     '%s_time.txt' % seq_name)
            ...
            begin_index = np.where(anno.any(1)==True)[0][0]
            # valid_index = anno.any(1)
            # seq_ious = seq_ious[valid_index]
            seq_ious = seq_ious[1:]
            # seq_times = seq_times[valid_index]
            ious[seq_name] = seq_ious
                # store sequence-wise performance
            ao, sr50, sr75, speed, _ = self._evaluate(
                seq_ious, seq_times)
            print('{},{},{},{}'.format(ao, sr50, sr75, speed))
            # save videos
            if save_video:
                ...
                image = load_pth(events[0]).astype(np.uint8)
                img_W, img_H = image.shape[0],image.shape[1]
                out_video = cv2.VideoWriter(video_file,
                                            cv2.VideoWriter_fourcc(*'MJPG'), 10,
                                            (img_W, img_H))
                for ith, (img_file, pred) in enumerate(zip(events, boxes[0])):
                    img = load_pth(img_file).astype(np.uint8)
    ...
    def report(self, tracker_names, plot_curves=True):
        ...
        if self.subset == 'test':
            ...
            return None
        elif self.subset == 'val' or self.subset=='train':
