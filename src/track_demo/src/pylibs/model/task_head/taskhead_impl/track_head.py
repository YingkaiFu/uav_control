...
class DenseboxHead(ModuleBase):
    ...
    def __init__(self):
        ...
        self.bottleneck=nn.Conv2d(head_numbers, down_number, kernel_size=1)
        self.bottleneck1=nn.Conv2d(head_numbers, down_number, kernel_size=1)
        ...
        self.query_embed = nn.Embedding(1, down_number)
        self.query_embed1 = nn.Embedding(1, down_number)
        self.transformer = Transformer(d_model=down_number,nhead=4,num_encoder_layers=3,num_decoder_layers=3,
                                       normalize_before=False,divide_norm=False)
        self.transformer1 = Transformer_encoder(d_model=down_number,nhead=4,num_encoder_layers=2,num_decoder_layers=0,
                                       normalize_before=False,divide_norm=False)
        self.cls_convs = []
        # self.self_attention = SimplifiedScaledDotProductAttention(d_model=head_numbers,h=8)
        self.box_head = Corner_Predictor(inplanes=down_number, channel=down_number, feat_sz=17, stride=17.8)
        # self.box_head = Corner_Predictor_Lite_Rep(inplanes=head_numbers, channel=head_numbers, feat_sz=17, stride=17.8)

        self.adjust = conv_bn_relu(head_numbers,down_number,kszie=3,stride=2,pad=0,has_bn=False,has_relu=True).cuda()
        self.bbox_convs = []
...
#         print(cls.shape,c_x.shape)
 ...
 # short_cut
        ...
        # run the corner head
        outputs_coord = box_xyxy_to_cxcywh(self.box_head(opt_feat1)) 
        outputs_coord_new = outputs_coord.view(bs, Nq, 4)
        out = {'pred_boxes': outputs_coord_new}
        # print(out)
        return out, outputs_coord_new
