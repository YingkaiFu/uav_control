...
from track_demo.msg import Track_Info
...
def callback(data):
    ...
    g_image = array.reshape(720, 1280, 1)
...
def showImage(data_sub, show_sub, camera_matrix, kcf_tracker_h, uav_id, conf):
    ...
    rospy.Subscriber(data_sub, numpy_msg(Floats), callback, queue_size=1)
    ...
    pub = rospy.Publisher(
        "/object_detection/siamfcpp_tracker", Track_Info, queue_size=1)
    ...
    while not rospy.is_shutdown():
        ...
        if start is False and draw_bbox.finish_event():
            ...
            target_pos = np.array(
                [(mouse_bbox[0] + mouse_bbox[2]) / 2, (mouse_bbox[1] + mouse_bbox[3]) / 2])
            target_sz = np.array(
                [mouse_bbox[2] - mouse_bbox[0], mouse_bbox[3] - mouse_bbox[1]])
            if (target_sz[0]**2 + target_sz[1]**2) < 100:
                continue
            boxes = np.array([mouse_bbox[0], mouse_bbox[1], mouse_bbox[2] -
                             mouse_bbox[0], mouse_bbox[3]-mouse_bbox[1]]).astype(int)
            pipeline.init(image, boxes)
            last_cx = boxes[0]+boxes[2]/2
            last_cy = boxes[1]+boxes[3]/2
        ...
        if start is True:
            ...
            cv2.rectangle(
                image_show, (res[0], res[1]), (res[0] + res[2], res[1] + res[3]), (0, 255, 255), 2)
            d_info.bboxs = res  # x,y,w,h
            print(res)
            bbox_cy = res[0] + res[2]/2
            bbox_cx = res[1] + res[3]/2
            d_info.y_dist = bbox_cy  # cy
            if 720>res[1]>0 and 1280>res[0]>0 and abs(last_cx-bbox_cx)<200 and abs(last_cy-bbox_cy)<200:
        ...
        cv2.putText(image_show, 'Double click to cancel the selection',
                    (10, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 1)
        if flag_lose is True:
            cv2.putText(image_show, 'target lost', (20, 40),
        ...
        if draw_bbox.show_draw:
            cv2.rectangle(
                image_show, draw_bbox.down_xy, draw_bbox.cur_draw_xy, (
                    0, 255, 0), 2
            )
...
if __name__ == '__main__':
    ...
    config = rospy.get_param(
        '~camera_info', 'src/track_demo/config/tracker/camera_param_gazebo_monocular.yaml')
    # tracker_conf = rospy.get_param('~tracker_conf','/home/yingkai/sdk_ros_warp/src/metavision_player/conf/img_ext_dataset.yaml')
    tracker_conf = rospy.get_param(
        '~tracker_conf', 'src/track_demo/config/tracker/img_ext_dataset.yaml')
    ...
    showImage(data_sub, show_sub, camera_matrix,
              kcf_tracker_h, uav_id, tracker_conf)
