...
inline size_t makeSliceImage(cv::Mat &frame_bgr, InputIt begin, InputIt end, Metavision::timestamp current_ts_us,
    ...
    const cv::Vec3b pos_event(1, 1, 1);
    const cv::Vec3b neg_event(-1, -1, -1);
    const cv::Vec3b bg_event(0, 0, 0);
    frame_bgr.setTo(bg_event);

    for (auto ev = slice_it.first; ev != slice_it.second; ++ev) {
        if (ev->p == 1) {
            frame_bgr.at<cv::Vec3b>(ev->y, ev->x) += pos_event;
        } else if (ev->p == 0) {
            frame_bgr.at<cv::Vec3b>(ev->y, ev->x) += neg_event;
        }
    }
...
template<typename InputIt>
inline size_t makeColorSliceImage(cv::Mat &frame_bgr, InputIt begin, InputIt end, Metavision::timestamp current_ts_us,
                             Metavision::timestamp accumulation_time_us, Metavision::timestamp frame_duration_us,
                             int display_fps, const Metavision::ColorPalette &palette) {
    auto slice_it = getSlice(begin, end, current_ts_us - accumulation_time_us, current_ts_us);
...
template<typename InputIt>
inline size_t makeSliceTwoImage(cv::Mat &frame_show, cv::Mat &frame_data, InputIt begin, InputIt end, Metavision::timestamp current_ts_us,
                             Metavision::timestamp accumulation_time_us, Metavision::timestamp frame_duration_us,
                             int display_fps, const Metavision::ColorPalette &palette) {
    auto slice_it = getSlice(begin, end, current_ts_us - accumulation_time_us, current_ts_us);
    ...
    const cv::Vec3b pos_color = getCVColor(palette, Metavision::ColorType::Positive);
    const cv::Vec3b neg_color = getCVColor(palette, Metavision::ColorType::Negative);
    // const cv::Vec<char,1> pos_event(1);
    // const cv::Vec<char,1> neg_event(-1);
    const cv::Vec3b bg_event(0, 0, 0);
    frame_show.setTo(bg_color);
    frame_data.setTo(0);
    for (auto ev = slice_it.first; ev != slice_it.second; ++ev) {
        if (ev->p == 1) {
            frame_data.at<char>(ev->y, ev->x) += 1;
            // if(frame_data.at<char>(ev->y, ev->x)>20){
            //     ROS_INFO("128!");
            // }
            frame_show.at<cv::Vec3b>(ev->y, ev->x) = pos_color;
            ...
            //     ROS_INFO("-128!");
            // }
            frame_show.at<cv::Vec3b>(ev->y, ev->x) = neg_color;
        }
    }
